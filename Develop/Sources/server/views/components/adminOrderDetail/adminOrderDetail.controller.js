...
function adminOrderDetailController($scope,$stateParams,dataService, $http, config, $rootScope){
        ...
        dataService.getDataServer(urlBase)
            .then(function (rs) {
                ...
                for(var i = 0; i < confirmationcode.length;i++){
                    ...
                    if(confirmationcode[i].typeid ==3){
                        $scope.codeInStock = confirmationcode[i].codecontent;
                    };

                    if(confirmationcode[i].typeid ==4){
                        $scope.codeOutStock = confirmationcode[i].codecontent;
                    };
                ...
                dataService.getDataServer(config.baseURI + '/api/store/' + $scope.order.storeid)
                    .then(function (rs) {
                        $scope.selectedStore =  rs.data;
                        console.log(rs);
                    })
            })
...
adminOrderDetailController.$inject = ['$scope','$stateParams','dataService','$http','config','$rootScope'];

angular.module('app').controller('adminOrderDetailController',adminOrderDetailController);
